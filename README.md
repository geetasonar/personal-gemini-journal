...
## 4. One-Click Vercel Deployment Guide

You can deploy Personal-Gemini-Journal to **Vercel** in under 60 seconds with full frontend and serverless API support (`/api/chat`, `/api/summarize`, `/api/health`):

### Option A: Deploy via GitHub & Vercel Dashboard

1. **Push your repository to GitHub**:
   - In AI Studio, click **Settings / Export** → **Push to GitHub** (or push from your local machine).
2. **Import to Vercel**:
   - Go to [vercel.com/new](https://vercel.com/new) and log into your Vercel account.
   - Select your GitHub repository (`personal-gemini-journal`).
...
4. **Add Environment Variables**:
   In the Vercel project configuration page, add:
   - `GEMINI_API_KEY`: Your Gemini API key from [Google AI Studio](https://aistudio.google.com/app/apikey).
5. **Deploy**:
   - Click **Deploy**. Vercel will build the frontend into `dist/` and automatically deploy `/api/chat.ts`, `/api/summarize.ts`, and `/api/health.ts` as serverless functions.
   - Vercel will output your custom permanent HTTPS production URL (e.g., `https://personal-gemini-journal-xxxx.vercel.app`).
6. **Add Vercel Domain to Firebase Authorized Domains**:
   - In the [Firebase Console](https://console.firebase.google.com/) under **Authentication** → **Settings** → **Authorized domains**, click **Add domain**.
   - Add your newly generated Vercel domain (`*.vercel.app` or custom domain) so Google Sign-In popups authenticate properly.
...
vercel --prod -e GEMINI_API_KEY="YOUR_GEMINI_API_KEY"
```

---

## 5. Comprehensive Functional Test Walkthroughs
